...
export const SEARCH_FILTER_DEFS: SearchFilterDef[] = [
  ...
  // 통합검색 개편 스펙의 종류 순서 목록엔 없지만(자료실 L2에서 이슈 섹션이 L1으로
  // 승격되며 빠진 것으로 추정) issues 테이블·/dashboard/issues 라우트가 여전히
  // 살아있어 검색 커버리지를 줄이지 않기 위해 마지막 순번으로 유지했다.
...
/** 검색 결과 섹션 표시 순서(고정) — 통합검색 개편 스펙(2026-08-09)의 종류 버튼 순서와 동일:
 *  핵심인사이트 → 키워드 → 기업동향 → 뉴스 → 유튜브 → 기술 Blog → AI 리포트 → 컨설팅 리포트 →
 *  공시자료. '이슈'는 스펙 목록엔 없지만 검색 커버리지 유지를 위해 마지막에 둔다(위 주석 참고). */
export const SEARCH_SECTION_ORDER: SearchFilterKey[] = [
  'insight', 'keyword', 'company', 'news', 'youtube', 'web-insight', 'ai-report', 'consulting-report', 'disclosure', 'issue',
]
