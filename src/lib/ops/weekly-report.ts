...
export async function gatherWeeklyReport(admin: SupabaseClient): Promise<WeeklyReport> {
  ...
  const [failed, partial, total, published, rejected, prevTotal, backlog, failedJobs, sourceRows, users, newUsers, pendingUsers, inactive, llm, settings, opened, resolved, stillOpen] = await Promise.all([
    ...
    admin.from('contents').select('id', { count: 'exact', head: true }).gte('collected_at', start).is('deleted_at', null),
    admin.from('contents').select('id', { count: 'exact', head: true }).gte('collected_at', start).eq('status', 'published').is('deleted_at', null),
    admin.from('contents').select('id', { count: 'exact', head: true }).gte('collected_at', start).eq('status', 'rejected').is('deleted_at', null),
    admin.from('contents').select('id', { count: 'exact', head: true }).gte('collected_at', previous).lt('collected_at', start).is('deleted_at', null),
    admin.from('contents').select('id', { count: 'exact', head: true }).is('body_fetched_at', null).not('original_url', 'is', null).is('deleted_at', null),
