...
/**
 * provider 가 한도·인증·결제·서버오류(429/401/402/403/5xx)로 소진 — 재시도 무의미, 즉시 다음 provider 로.
...
export class LlmRateLimitError extends Error {
  constructor(
    ...
    super(`${providerName}: rate limited (429/401/402/403/5xx)`)
    this.name = 'LlmRateLimitError'
  }
}

/** provider 모델이 영구적으로 사용 불가(400/404/422) — 같은 모델로 재시도해도 결과가 같다. 쿨다운 대상 아님(모델 문제이지 한도 문제가 아님). */
export class LlmModelUnavailableError extends Error {
  constructor(
    public readonly providerName: string,
    public readonly status?: number,
  ) {
    super(`${providerName}: model unavailable (400/404/422)`)
