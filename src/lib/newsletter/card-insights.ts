...
export interface CardInsight {
  /** 왜 지금 봐야 하는가 — 시장/경쟁 맥락. 2문장, 120~160자. 기사가 U+ B2B 와 무관하면 null. */
  why: string | null
  /** LG U+ B2B 관점 시사점·액션. 3문장, 160~220자. U+ 와 실제로 이어지는 지점이 없으면 null. */
  action: string | null
}
...
- why 에 기사에 등장하지 않는 주체(국내 기업, 통신사, 정부 등)를 새로 끌어들이지 않는다.
  기사에 없는 주체를 판단의 근거로 쓰지 않는다.

[탈출구 — 억지로 채우지 않는다]
- LG유플러스 B2B 와 실제로 이어지는 지점이 없으면 "action": null 을 낸다.
  억지로 방향을 만들어내는 것보다 없는 편이 낫다.
- 기사가 LG유플러스 B2B 와 아무 접점이 없으면 "why": null, "action": null 을 낸다.
...
- 제목·요약 문장을 그대로 옮기거나 말만 바꿔 쓰지 않는다.
  나쁜 예: "요약: A사가 B를 인수했다" → "짚어보면: A사의 B 인수는…" (요약을 주어만 바꿔 되풀이한 것 — 금지)
...
function parseInsightItem(item: RawInsightItem): CardInsight | null {
  ...
  const action = toNullableString(item.action, 400)

  // why·action 이 둘 다 없으면 인사이트 없음으로 취급 — 호출부(prepareNewsletterIssue)가
  // 이 카드를 메일에서 제외한다(지시서 20260902 §9-5).
