  ...
  try {
    const result = await runJob(admin, { key: 'cron:competitor-weekly', trigger: 'cron' }, async () => {
      if (!LLM_PROVIDERS.some(p => p.isConfigured())) {
        return { ok: true, skipped: 'llm_not_configured' }
      }
      ...
      if (dow !== settings.generate_dow || hour !== settings.generate_hour) {
        return { ok: true, skipped: 'not_scheduled', dow, hour }
      }
      ...
      if (existing) {
        return { ok: true, skipped: 'already_exists', weekStart }
      }

      // ── 4. 생성 — auto_publish=false 면 애초에 draft 로 저장(published 창 없음, 284 재현검증 수정) ──
      const deadline = Date.now() + 270_000
      const genResult = await generateCompetitorWeeklyReport(admin, { deadline, publish: settings.auto_publish })

      console.log('[크론/competitor-weekly] 완료:', JSON.stringify(genResult))
