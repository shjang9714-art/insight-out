 ...
 * 498: crawl(phase='sources')과 crawl-seeds(phase='seeds') 로 크론이 갈라지면서, 이
 * 웜업은 두 잡 중 나중에 도는 쪽(crawl-seeds, 05:40 KST)이 맡는다 — trending_keywords 는
 * 소스 폴링과 seed 검색 결과를 모두 반영해야 하므로 먼저 도는 crawl 이 웜업하면 seed
 * 수집분이 빠진 채로 캐시가 굳는다.
 */
async function warmTrendingCache(): Promise<void> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      console.warn(
        `[크론/crawl-seeds] 급상승 캐시 웜업 시도 ${attempt + 1}/3 — 기준일 아직 "${result?.asOfDateKst}"(오늘 "${today}" 아님)`,
      )
    } catch (e) {
      console.error(`[크론/crawl-seeds] 급상승 캐시 웜업 시도 ${attempt + 1}/3 실패:`, e)
    }
  }
}
...
// 498: 키워드/회사 seed 검색(순차 처리, 아이템당 dedup 쿼리 최대 3회)만으로 210초 안팎이 걸린다
// (2026-07-12 §5 재검증). runCrawl 내부 소프트 데드라인이 이 300초 안에서 우아하게 멈춘다 —
// 하드킬로 job_runs 미기록되는 사고를 방지.
export const maxDuration = 300

/**
 * GET /api/cron/crawl-seeds
 *
 * Vercel Cron 이 매일 KST 05:40 (UTC 20:40) 에 호출.
 * Vercel 은 CRON_SECRET 이 설정된 경우 Authorization: Bearer <CRON_SECRET> 헤더를 자동 첨부.
 * 외부 직접 호출 차단을 위해 반드시 인증 검증.
 * 498: 키워드/회사 seed 검색만 담당한다(소스 폴링은 /api/cron/crawl 이 phase='sources' 로 처리).
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const summary = await runJob(admin, { key: 'cron:crawl-seeds', trigger: 'cron' }, async () => {
      ...
      return runCrawl({ phase: 'seeds', ...options })
    })
    ...
    try {
      ...
      console.error('[크론/crawl-seeds] 급상승 캐시 무효화/웜업 실패(무시하고 계속):', e)
    }
    ...
    console.error('[크론/crawl-seeds] 크롤러 실행 오류:', err)
