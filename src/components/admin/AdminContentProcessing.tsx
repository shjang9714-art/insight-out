...
function JobCard({ job, description, children, isRunning, help }: JobCardProps) {
  return (
...
export default function AdminContentProcessing({ jobs }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <JobCard job={canonicalJob} isRunning={isRunning(canonicalJob.key)} description="구글뉴스 등 리다이렉트 주소를 실제 원문 주소로 정리합니다.">
        <Button type="button" size="sm" variant="outline" disabled={isRunning(canonicalJob.key)} onClick={() => startJob(canonicalJob.key)}>
          원문 URL 정규화
        </Button>
      </JobCard>

      <JobCard job={thumbnailJob} isRunning={isRunning(thumbnailJob.key)} description="썸네일이 없는 크롤 뉴스·웹인사이트의 원문 og:image를 다시 받아옵니다.">
      ...
      <JobCard job={transcriptJob} isRunning={isRunning(transcriptJob.key)} description="유튜브 영상의 자막을 수집해 한글로 번역합니다. 비공식 엔드포인트를 순차 호출하므로 rate limit에 주의하세요.">
      ...
      <JobCard job={pdfCoverJob} isRunning={isRunning(pdfCoverJob.key)} description="업로드된 PDF의 첫 페이지를 표지로 설정합니다. 표지가 이미 있으면 건너뜁니다.">
      ...
      <JobCard job={clusterJob} isRunning={isRunning(clusterJob.key)} description="제목만으로 못 묶인 같은 사건 기사를 본문 유사도로 재평가해 병합하고, 신호 기반으로 대표를 재선정합니다.">
