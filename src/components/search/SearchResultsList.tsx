...
function renderItem(item: UnifiedResult) {
  ...
  if (item.source === 'entities' && item.entity) {
    ...
    return <SearchResultCard key={item.key} href={`/dashboard/entities/${entity.id}`} title={entity.canonical_name} excerpt={excerpt} publishedAt={entity.latestPublishedAt} typeBadge={{ label: def.label, className: def.badgeClass }} />
  }
  if (item.source === 'keywords' && item.keyword) {
    const keyword = item.keyword
    const def = SEARCH_FILTER_DEFS.find(d => d.key === 'keyword')!
    return <SearchResultCard key={item.key} href={`/dashboard/keywords/${encodeURIComponent(keyword.name)}`} title={keyword.name} excerpt={`연결된 기사 ${keyword.linkedCount}건`} publishedAt={keyword.latestPublishedAt} typeBadge={{ label: def.label, className: def.badgeClass }} />
  }
  return null
}

export default function SearchResultsList({ sections }: { sections: SearchSection[] }) {
  return (
