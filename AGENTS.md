...
**1 작업 사이클:**
1. **Opus(Cowork)** — 지시서 작성 (`docs/sonnet-지시서/NN.md`, 로컬, **커밋 X**). 스키마/SQL 산출물도 작성. Opus 는 git 을 실행하지 않는다(샌드박스 제약).
2. **David** — Claude Code 에 위임(한 줄).
3. **Sonnet(Claude Code)** — 구현 + `tsc`/`build`/`lint` + 완료 보고. **이 시점엔 커밋·푸시하지 않는다.**
4. **David** — Sonnet 보고를 Opus 에 전달.
5. **Opus** — 검증(재현) → ✅ 통과 / 🔧 수정요청.
6. **✅ 시** — David 가 "커밋·푸시해" 트리거 → **Sonnet 이 지시서+코드를 한 번에 커밋·푸시** → `/api/version` 으로 배포 확인. (1 사이클 끝)
