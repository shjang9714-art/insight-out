...
#### Opus↔Sonnet 협업 루프 (정본 — 새 세션은 이 순서를 따른다)

**1 작업 사이클:**
1. **Opus(Cowork)** — 지시서 작성 (`docs/sonnet-지시서/NN.md`). 스키마/SQL 산출물도 작성. Opus 는 git 을 실행하지 않는다(샌드박스 제약).
2. **David** — 지시서를 **즉시 origin/main 에 커밋·푸시**(구현자에게 넘기기 **전에**). `git add docs/sonnet-지시서/NN.md [docs/sql-handoff/…] && git commit -m "docs: 지시서 NN 추가" && git push origin main`. ⚠️ **지시서를 미커밋 로컬 파일로 두지 않는다** — 브랜치/워크트리 churn 으로 **유실·비가시**(구현자는 origin/main 기준으로 본다). 이게 지켜지지 않으면 구현자가 지시서를 못 보고 범위를 임의 재구성한다(실제 사고 이력 있음).
3. **David** — Claude Code 에 위임(한 줄).
4. **Sonnet(Claude Code)** — 구현 + `tsc`/`build`/`lint` + 완료 보고. **이 시점엔 코드 커밋·푸시하지 않는다**(지시서는 이미 2단계에서 main 에 있음).
5. **David** — Sonnet 보고를 Opus 에 전달.
6. **Opus** — 검증(재현) → ✅ 통과 / 🔧 수정요청.
7. **✅ 시** — David 가 "커밋·푸시해" 트리거 → **Sonnet 이 코드를 브랜치에 커밋·푸시 → PR → 머지** → `/api/version` 으로 배포 확인. (1 사이클 끝)
...
- **지시서는 위임 전 커밋하지 않는다** — Sonnet 이 로컬 파일을 직접 읽으므로. 코드와 함께 ⑥에서 커밋.
- **검증(⑤) 통과 전 푸시 금지.** 단 **수희가 Supabase 에서 실행할 SQL 핸드오프**는 예외적으로 먼저 커밋·푸시해 수희가 받게 한다.
- 배포 확인은 항상 `/api/version`(상세: `docs/배포-검증-프로세스.md`).
...
### 20.8 작업 후 기록 (필수) — 담당: **Sonnet(Claude Code)**

#### ⓪ ⭐ **git 커밋 메시지가 정본이다 — 완료는 커밋 메시지에 박는다**

> **David: *"완료시에는 작업번호와 내용을 항상 명기해 메모리에 박제해"***
> 박제 위치는 **커밋 메시지**다. **작업번호(지시서 NNN)와 내용을 커밋 메시지에 반드시 명기**한다.

```
type: 지시서 NNN <제목> — <무엇을 왜 고쳤는지 한국어 요약>
```

⛔ **`docs/작업계획서.md` 는 절대 건드리지 말 것 (David 전용).**
- 구현 에이전트(Sonnet)는 `docs/작업계획서.md` 를 **생성·수정·커밋하지 않는다.** push 후 자동으로 한 줄 추가하는 이전 루틴은 **폐지**됐다.
- 이 파일 갱신은 **David 가 필요할 때 직접** 한다. "작업계획서에 기록해줘" 같은 **명시적 지시가 있을 때만** 손댄다.
- 완료 기록의 3요소(지시서 번호·내용·커밋 SHA)는 **커밋 메시지**로 충족한다 — 작업계획서.md 로 충족하는 것이 아니다.

- **지시서 없는 작업**(핫픽스·오타)은 `지시서 없음` 이라고 **명시**한다. 비워 두지 않는다.
- **폐기·방향 전환도 기록한다.** *"안 했다"* 도 결정이다.
- ⛔ **David 의 트리거를 기다리지 말 것.** ⓪는 push 의 일부다.

#### ①② Notion 동기화 — David 트리거 시

> Notion(① 개발 일지 · ② 작업 DB · 신호등) 동기화는 **Sonnet 이 수행**한다(Claude Code 에 Notion 커넥터 연결됨). Opus(Cowork)는 직접 하지 않는다.
> **표준 루틴**: [`docs/기록-동기화-루틴.md`](./기록-동기화-루틴.md). David 트리거 = `"기록 동기화 루틴대로 동기화해줘"`. 예외: 커넥터 장애 시에만 Opus 대행.

①은 동기화 시 모든 push 에 대해, ②는 작업 DB 항목이 있을 때만.
...
4. **완료 기록은 커밋 메시지에** (§20.8 ⓪). `docs/작업계획서.md` 는 **건드리지 않는다**(David 전용). Notion 동기화는 David 트리거 시에만:
   - **Opus(Cowork)**: 무엇을 기록할지 "기록 지시서"(Notion 페이지/DB id·값)를 작성해 David 에게 넘긴다. 직접 Notion 기록·작업계획서 수정은 하지 않는다.
   - **Sonnet(Claude Code)**: David 트리거 시 지시서대로 Notion(개발 일지 ① + 작업 DB ②)을 갱신하고 결과 URL 보고. **`docs/작업계획서.md` 는 자동 갱신하지 않는다.** (Claude Code 에 Notion 커넥터 연결됨)
   - 두 단계: ① 개발 일지 추가(David 트리거 시), ② 작업 DB 상태 갱신(해당 항목 있을 때만).
