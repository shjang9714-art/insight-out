...
#### ⓪ ⭐ **git 이 정본이다 — push 하면 즉시, 트리거 없이 박는다**

> **David: *"완료시에는 작업번호와 내용을 항상 명기해 메모리에 박제해"***
> 이게 **1순위**다. Notion(①②)은 그 다음이고, **트리거를 기다린다.**

**push 직후, `docs/작업계획서.md` 에 한 줄을 추가한다. 같은 커밋에 넣는다.**

```
- **지시서 NNN <제목> 🟢 (YYYY-MM-DD)**: <무엇을 왜 고쳤는지 1~3문장. 남이 6주 뒤에 읽어도 알게>. · `<SHA>`
```

**필수 3요소 — 하나라도 빠지면 기록이 아니다:**

| | 없으면 |
|---|---|
| **지시서 번호** | **어느 지시서가 살아있고 어느 게 죽었는지 알 수 없다.** 오늘 298·314가 "준비됨"으로 남아 있었는데 파일이 없었다 |
| **내용(왜)** | 6주 뒤에 되살아난다. **폐기·결정은 사유가 곧 방어막이다** |
| **커밋 SHA** | 되돌릴 수가 없다 |
